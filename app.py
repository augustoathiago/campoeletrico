...
with c1:
    ...
    x1 = st.slider("x₁ (m)", 0.0, 16.0, 2.0, 0.1)
    y1 = st.slider("y₁ (m)", 0.0, 16.0, 2.0, 0.1)
    q1 = st.slider("q₁ (µC)", -5.0, 5.0, 2.0, 0.05) * 1e-6

with c2:
    st.subheader("Partícula 2")
    x2 = st.slider("x₂ (m)", 0.0, 16.0, 14.0, 0.1)
    y2 = st.slider("y₂ (m)", 0.0, 16.0, 2.0, 0.1)
    q2 = st.slider("q₂ (µC)", -5.0, 5.0, -2.0, 0.05) * 1e-6

with c3:
    st.subheader("Ponto P")
    xP = st.slider("xₚ (m)", 0.0, 16.0, 8.0, 0.1)
    yP = st.slider("yₚ (m)", 0.0, 16.0, 8.0, 0.1)
...
st.latex(r"\vec{E}_1 = K\frac{q_1}{r_1^2}\,\hat{r}_1")
st.latex(r"\vec{E}_2 = K\frac{q_2}{r_2^2}\,\hat{r}_2")
...
// Grade + ticks numerados
ctx.strokeStyle="#eee";
{''.join([f'''
ctx.beginPath();ctx.moveTo(X({t}),Y(0));ctx.lineTo(X({t}),Y(16));ctx.stroke();
ctx.beginPath();ctx.moveTo(X(0),Y({t}));ctx.lineTo(X(16),Y({t}));ctx.stroke();
ctx.fillText("{t}",X({t})-6,Y(0)+16);
ctx.fillText("{t}",X(0)-22,Y({t})+4);
''' for t in ticks])}

// Eixos
ctx.strokeStyle="#000"; ctx.lineWidth=2;
ctx.beginPath(); ctx.moveTo(X(0),Y(0)); ctx.lineTo(X(16),Y(0)); ctx.stroke();
ctx.beginPath(); ctx.moveTo(X(0),Y(0)); ctx.lineTo(X(0),Y(16)); ctx.stroke();

ctx.fillText("x (m)",X(16)+10,Y(0)+5);
ctx.fillText("y (m)",X(0)-10,Y(16)-10);
...
// Vetor com componentes tracejadas
